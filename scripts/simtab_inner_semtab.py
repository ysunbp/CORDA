...
class SimTAB(nn.Module):
    ...
    def forward(self, y1, y2, clsA, clsB, cls_idx_A=None, flag=True, da=None, cutoff_ratio=0.1):
        if flag:
            # pre-training
            # encode
            batch_size = len(y1)
            ...
            non_negative = torch.nonzero(clsA >= 0)
            enc = self.projector(self.bert(x1)[0][non_negative[:, 0], non_negative[:, 1]]) # (batch_size, emb_size)
            out = self.fc(enc)
            return out
