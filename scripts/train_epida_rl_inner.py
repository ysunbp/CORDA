...
import os
import torch
from .simtab_inner import SimTAB
...
from generate_training import Token_Augmenter
...
def train(trainset_nolabel, train_set, valid_set, test_set, hp):
    ...
    optimizer_f = AdamW(model.parameters(), lr=hp.f_lr) # update the optimizer for fine-tuning
...
def epida_sent_rl(base_col, label, hp, model, augmenter, tokenizer, policy_set, epsilon): # generate and select the high quality content augmentation
    da_size = hp.da_size
    ...
    for aug_c in augment_content:
        labels.append(label)
        CLSs.append(augmenter.update_cur_CLS(aug_c))
    ...
    for idx in sortargs[:da_size]:
        if not idx == 0:    
            ...
            if content_policy[idx-1] in policy_set.keys():
                ...
                policy_set[content_policy[idx-1]] = total_scores[idx]
    return new_content, policy_set, out_CLSs
...
def update_dataloader_sent(train_iter, hp, model_save_path, epsilon, policy_set):
    ...
    for i, batch in enumerate(tqdm(train_iter)):
        x, y, _ = batch
        x = x[0].tolist()
        augmented_content, policy_set, cls_set = epida_sent_rl(x, y, hp, model, augmenter, tokenizer, policy_set, epsilon)
        Xs += augmented_content
        ys += [y for i in range(len(augmented_content))]
        clss += cls_set
    return Xs, ys, clss, policy_set
...
def finetune_epida_rl(train_set, valid_set, test_set, hp, epsilon):
    ...
    for epoch in range(hp.n_epida_epochs):
        ...
        xs, ys, clss, policy_set = update_dataloader_sent(train_iter_in, hp, model_save_path, epsilon, policy_set)

        train_iter.dataset.update(xs, ys, clss)
