...
class Token_Augmenter(object):
    ...
    def update_out_CLS(self, tokens, cls_idx):
        cur_CLS = self.update_cur_CLS(tokens)
        out_indexes = []
        for idx in cls_idx[0]:
            if idx>=0:
                out_indexes.append(cur_CLS[idx])
        return out_indexes
    ...
    def augment(self, tokens, op='swap_token'):
        ...
        if op == 'del_span':
            ...
            if len(new_tokens) >= self.max_len:
                return tokens
            return new_tokens
        elif op == 'swap_token':
            new_tokens = tokens
